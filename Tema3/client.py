...
class FTPClient:
    ...
    def _fetch_server_files(self):
        """Helper: fetch the list of files from the server"""
    ...
    def _select_file_from_server(self):
        """Helper: list server files and ask the user to pick one"""
        files = self._fetch_server_files()
        if files is None:
            return None
        if not files:
    ...
    def read_file(self):
        ...
        filename = self._select_file_from_server()
        if not filename:
            return
    ...
    def download(self):
        ...
        try:
            with open(local_path, 'w') as f:
                f.write(content)
            print(f"✓ File '{filename}' saved to {LOCAL_FILES_DIR}/ ({len(content)} bytes)")
        except Exception as e:
    ...
    def see_file_operation_history(self):
        ...
        for i, entry in enumerate(history, 1):
            print(f"  {i}. [{entry['timestamp']}] {entry['user']} - "
                  f"{entry['operation'].upper()}: {entry['details']}")
