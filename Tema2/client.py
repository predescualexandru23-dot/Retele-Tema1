...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE: Trebuie sa furnizezi un mesaj. Exemplu: PUBLISH salut tuturor")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if not este_conectat:
            ...
            continue
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE: Trebuie sa furnizezi un ID. Exemplu: DELETE 3")
            continue
        argument = parti[1].strip()
        try:
            int(argument)
        except ValueError:
            print(f"EROARE: ID-ul '{argument}' nu este un numar intreg valid.")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
